...
def CheckListInclu(listeRef, liste):
    ...
    if isinstance(listeRef, list):
        if len(listeRef) >1:
            res = Check(listeRef[0], liste) and CheckListInclu(listeRef[1:], liste)
        else:
            res = Check(listeRef, liste)
    else:
...
print ("Nombre d'applicants :", len(set(Inventeurs)))
for app in Applicants:
    
    if app not in AppDejaVus:
        reste = Applicants- set([app]+AppDejaVus)
        
        AppDejaVus.append(app)
        for app2 in reste:
            if fuzz.token_sort_ratio(app, app2)>89:
                if len(app) == len(app2):
                    print ("Suspect traité : ", app, app2)
                AppDejaVus.append(app2)
                if app in Applicant_Norm.keys():
                    Applicant_Norm [app].append(app2)                    
                else:
                    Applicant_Norm [app] = [app2]
                if app2 not in BadCasApp.keys():
                    BadCasApp [app2] = app
