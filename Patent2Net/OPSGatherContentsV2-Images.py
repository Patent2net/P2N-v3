...
def extract_best_images(docs_instance):
    ...
    if isinstance(docs_instance, list):
        for doc in docs_instance:
            name = doc['@desc'].lower()
            if name == 'drawing':  # reset anything before and return this as better images results
                chosen_doc = doc
                break
    ...
    link = chosen_doc['@link']
    page_start = int(chosen_doc['ops:document-section']['@start-page'])
    num_pages = int(chosen_doc['@number-of-pages'])
