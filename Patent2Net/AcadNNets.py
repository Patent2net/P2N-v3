...
project = RepDir
if 'AcadCorpora' not in os.listdir(configFile.ResultPath): # special condition for P7
...
Inventeurs = set()
# =============================================================================
# Récupération des auteurs et des stats
# 
# =============================================================================
Auteurs = dict()
...
for lig in DataMatch:
    ...
    auteur.append(col [0])
    ####
    ### sauvegarde des inversions noms prenoms
    for aut in itertools.permutations(tempo):
        auteur .append(' ' .join(aut))

    
    for form in set(auteur):
        if form not in Auteurs.keys():
            Auteurs [form] = dict()
            Auteurs [form]["publisMatch"] = int(col [1])
            Auteurs [form]["publis"] = int(col [2])
            Auteurs [form]["scoreMoy"] = float(col [3])
            PubMedAuct.append(form)
        else:
            if int(col [1]) > Auteurs [form]["publisMatch"]:
                Auteurs [form]["publisMatch"] = int(col [1])
            if int(col [2]) > Auteurs [form]["publis"]:
                Auteurs [form]["publisMatch"] = int(col [2])
                Auteurs [form]["scoreMoy"] = float(col [3])  
            PubMedAuct.append(form)
...
for lig in DataPasMatch:
    ...
    for form in set(auteur):
        if form not in Auteurs.keys():
            Auteurs [form] = dict()
            Auteurs [form]["publisMatch"] = 0
            Auteurs [form]["publis"] = 0
            Auteurs [form]["scoreMoy"] = 0
            PubMedAuct.append(form)
        else:
            # case where some prod f the author do not match !
            pass
            #print (form)

    
with open(Auteur+'//traceAuct.csv', 'r', encoding = 'utf8') as fic:
...
for lig in dataAuct:
    ...
    for form in set(auteur):
        if form not in Auteurs.keys():
            Auteurs [form] = dict()
            Auteurs [form]['publis'] =  int(col [1])
            Auteurs [form]['publisMatch'] = int(col [2])
            Auteurs [form]['affilFr'] = col [3]
            Auteurs [form] ["scoreMoy"] = -1
        else: # merging
            Auteurs [form]['publis'] += int(col [1])
            Auteurs [form]['publisMatch'] += int(col [2])
            if 'affilFr' in Auteurs [form].keys():
                if Auteurs [form]['affilFr'] == 'True' or col [3] == 'True':
                    Auteurs [form]['affilFr'] = 'True'
                else:
                    Auteurs [form]['affilFr'] = 'False'
            Auteurs [form]['affilFr'] = col [3]
            if "scoreMoy" not in  Auteurs [form].keys():
                 Auteurs [form] ["scoreMoy"] = -1 # pour identifier les paumes

# Génération des auteurFr ou pas
# the following should be empty
NonVus = [Aut for Aut in Auteurs.keys() if 'affilFr' not in Auteurs[Aut].keys()]

# but I had to manually add a special line for author using '-' separator in their compound name
# that cases aren't taken in account
# Errata lines 213 and 214 are enought
...
if 'Description' + 'Families'+ndf in os.listdir(ResultBiblioPath):
    with open(ResultBiblioPath + '//' + 'Families'+ndf, 'r') as data:
        dico = LoadBiblioFile(ResultBiblioPath, 'Families'+ndf)
...
for bre in df.itertuples():
    ...
    if not isinstance(bre.equivalents, list):
        ...
        df.at [bre.Index, 'equivalents' ] = list (set([ipc for ipc in bre.equivalents if ipc.lower() not in ['empty', '', 'none']]))
    if not isinstance(bre.CitedBy, list):
        df.at [bre.Index, 'CitedBy' ] = [bre.CitedBy]
    else:
        df.at [bre.Index, 'CitedBy' ] = list (set([ipc for ipc in bre.CitedBy if ipc.lower() not in ['empty', '', 'none']]))            
    if not isinstance(bre.CitP, list):
        df.at [bre.Index, 'CitP' ] = [bre.CitP]
    else:
        df.at [bre.Index, 'CitP' ] = list (set([ipc for ipc in bre.CitP if ipc.lower() not in ['empty', '', 'none']]))            
    if not isinstance(bre.CitO, list):
        df.at [bre.Index, 'CitO' ] = [bre.CitO]
    else:
        df.at [bre.Index, 'CitO' ] = list (set([ipc for ipc in bre.CitO if ipc.lower() not in ['empty', '', 'none']]))     
...
# Techno = dict()
for bre in df.itertuples():
    #☺ saving inventors. I guess there's a best way
    # for appl in bre.inventor:
    #     Inventeurs. add(appl)
    #     if appl in Techno.keys():
    #         for cib in bre .IPCR11:
    #             Techno[appl].append(cib)
    #     else:
    #          Techno[appl] =  [cib for cib in bre .IPCR11 if cib not in Exclus]  
    ...
    for truc in bre .inventor:
        ...
        if truc not in Auteurs.keys() and truc.title() not in Auteurs.keys():
            #print ('GRRR ', truc)
            Candidate = []
            for cle in Auteurs.keys():
                if fuzz.token_set_ratio(cle, truc)>90:
                    Candidate.append(cle)
            #print (Candidate)
            if len(Candidate) == 1:
                Auteurs [truc] = Auteurs [Candidate [0]]
            elif len(Candidate) > 1:
                # in fact they are probably the same
                Auteurs [truc] = Auteurs [Candidate [0]]
            if len(Candidate) == 0:
                print (truc)
                Auteurs [truc] = {'publis': -1, 'publisMatch': 0, 'affilFr': 'False', "scoreMoy":0}
...
for aut in Inventeurs:
   ...
   # pas forcément vrai j'ai dû zapper cette info dans le traitement
    ...
    auteur = []
    typeAut = ''
    for auto in itertools.permutations(tempo1):
        auteur .append(' ' .join(auto))
    for auto in itertools.permutations(tempo2):
        auteur .append(' ' .join(auto))  
    ...
    if typeAut =='': #form.replace(' ','') not in lstFr and form.replace(' ','') not in lstEtr:    
       ...
       # elif form in AuteursFr:
       #     typeAut = 'AutFr'
       # else:
       #     typeAut = 'AutEtr'
    # hceck consistance ans adding match scores

    if aut in dicoAttrsAut.keys():
        if aut not in Auteurs.keys():
            for form in set(auteur):
                if form in Auteurs.keys():
                    dicoAttrsAut [aut] ['publis'] = Auteurs [form] ['publis']
                    dicoAttrsAut [aut] ['match'] = Auteurs [form] ['publisMatch']
                    dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [form] ["scoreMoy"]
                    if Auteurs [form] ['publis'] >0:
                        dicoAttrsAut [aut] ['category'] = typeAut
                        if dicoAttrsAut [aut] ['match'] >0:
                            dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [form] ["scoreMoy"]
                            
                        else:
                            dicoAttrsAut [aut] ['scoreMoy'] = 0
                    else:
                        dicoAttrsAut [aut] ['category'] ="PasSurPubMed"
                    

                # else:
                #     dicoAttrsAut [aut] ['publis'] = Auteurs [aut] ['publis']
                #     dicoAttrsAut [aut] ['match'] = Auteurs [aut] ['publisMatch']
                #     dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [aut] ["Score moyen"]
                #     dicoAttrsAut [aut] ['category'] = typeAut
        ...
        dicoAttrsAut [aut] ['publis'] = Auteurs [aut] ['publis']
        dicoAttrsAut [aut] ['match'] = Auteurs [aut] ['publisMatch']
        dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [aut] ["scoreMoy"]
        
        if Auteurs [aut] ['publis'] >0:
            dicoAttrsAut [aut] ['category'] = typeAut
            if dicoAttrsAut [aut] ['match'] >0:
                dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [aut] ["scoreMoy"]
            ...
            dicoAttrsAut [aut] ['category'] ="PasSurPubMed"
            dicoAttrsAut [aut] ['scoreMoy'] = 0
            dicoAttrsAut [aut] ['match'] = 0
            dicoAttrsAut [aut] ['publis'] = 0
...
for appl in Inventeurs:
    if len(appl)>0 and appl not in Exclus :
        ...
        dicoAttrsAut [appl] = {
                      'AutFr': dicoAttrsAut [appl] ['category'] == 'AutFr',
                      'publis' : dicoAttrsAut [appl] ['publis'],
                      'match': dicoAttrsAut [appl] ['match'],
                      'scoreMoy': dicoAttrsAut [appl] ['scoreMoy'], 
...
ligne +=    str(NbBrevets) +';'+\
        str(len(set(Applicants)))+';'+\
        str(len(Inventeurs)) +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"] =="AutFr"]))  +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"]=="AutEtr"])) +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["match"] >0]))+ ';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"]=="PasSurPubMed"])) +';'+\
        str(TailleFamiile) +';'+\
        str(nbCitations) +';'+\
        str(IPC11) +';'+\
        str(IPC7) +';'+\
        ...
        str(len(set(Applicants))/tailleCrp)+';'+\
        str(len(dicoAttrsAut.keys())/tailleCrp) +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"] =="AutFr"])/tailleCrp)  +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"]=="AutEtr"])/tailleCrp) +';'+\
        str(len([aut for aut in Inventeurs if dicoAttrsAut[aut]["category"]=="PasSurPubMed"])/tailleCrp)+ ';'+\
...
Networks["_ApplicantInventor"] = [configFile.ApplicantInventorNetwork, ["Applicant", "Public", "Private", 'AutFr', 'AutEtr', 'PasSurPubMed']] # GraphAuteursAppli
