...
for lig in DataMatch:
    ...
    for form in set(auteur):
        if form not in Auteurs.keys():
            ...
            Auteurs [form]["Score moyen"] = float(col [3])
        else:
...
for lig in dataAuct:
    ...
    for form in set(auteur):
        if form not in Auteurs.keys():
            ...
            Auteurs [form]['affilFr'] = col [3]
            if "Score moyen" not in  Auteurs [form].keys():
                 Auteurs [form] ["Score moyen"] = -1 # pour identifier les paumes
...
for bre in df.itertuples():
    ...
    for truc in bre .inventor:
        ...
        if truc not in Auteurs.keys() and truc.title() not in Auteurs.keys():
            print ('GRRR or not in PubMed ? ', truc)
            Auteurs [truc] = {'publis': -1, 'publisMatch': 0, 'affilFr': 'False'}
        ...
        if aut not in Auteurs.keys():
            for form in set(auteur):
                if form in Auteurs.keys():
                    ...
                    dicoAttrsAut [aut] ['scoreMoy'] = Auteurs [form] ["Score moyen"]
...
Networks["_ApplicantInventor"] = [configFile.ApplicantInventorNetwork, ["Applicant", "Inventor", 'AutFr', 'AutEtr', 'PasSurPubMed']] # GraphAuteursAppli
...
Networks["_LabelApplicants"] =  [configFile.ApplicantNetwork, [ 'label', "Applicant"]] # GraphBrevetsEquivalents#GraphBrevets
