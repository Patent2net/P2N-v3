...
#from P2N_Lib import dlPatentList, MessageAnnouncer
...
if nbTrouves == 0:
    announcer.announce(msg=dlPatentList (bar, maxi=100, prog_inc = 100, valeur = 100))
else:
    announcer.announce(msg=dlPatentList (bar, maxi=nbTrouves, prog_inc = 100/nbTrouves, valeur = 0))
    ...
    while len(lstBrevets) < nbTrouves and not STOP:
        ...
        for p in temp:
            if p not in lstBrevets:
                ...
                ajouts += 1
        if nbTrouves == 0:
            announcer.announce(msg=dlPatentList (bar, maxi=nbTrouves, prog_inc = 100*ajouts, valeur = ajouts))
        else:
            
            announcer.announce(msg=dlPatentList (bar, maxi=nbTrouves, prog_inc = 100*ajouts/nbTrouves, valeur = ajouts))
...
#dlPatentList (bar, maxi=nbTrouves, prog_inc = 100*ajouts/nbTrouves, valeur = ajouts)
...
#                    cPickle.dump(bre, ficRes)
    ...
    for brevet in lstBrevets:
        ...
        if 'invalid result' not in str(brevet) and 'document-id' in list(brevet.keys()):
            # nameOfPatent for file system save (abstract, claims...)
            if brevet['document-id']['kind']['$'].isdigit():
                brevet['document-id']['doc-number']['$'] = brevet['document-id']['doc-number']['$'] + brevet['document-id']['kind']['$']
                brevet['document-id']['kind']['$'] =''
